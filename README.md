...
|🧩 Feature|移除拖拽预览功能|chromium目前已实现拆分视图功能，拖拽链接至浏览器窗口两侧即可以拆分视图预览网页；而且由于油猴脚本的权限限制，点击父窗口自动关闭子窗口的功能无法完美实现（子窗口中的网页若发生重定向，会使子窗口丢失与父窗口的关联，导致用户点击父窗口无法关闭子窗口）。所以综合考虑直接移除拖拽链接预览网页功能是更好的选择|中|
