...
|🧩 Feature|增加中国大陆身份证号码校验功能|身份证号检验按钮弹出条件：1、脚本语言为中文；2、选区为16-20位纯数字（末尾字母X也视为数字）；3、开头两位数字为11-65|低|
|🧩 Feature|增加银行卡号校验功能|1、银联、VISA、MASTERCARD：利用Luhn算法校验银行卡号有效性；2、计划暂时仅支持银联：解析Bin信息（包括银行名称、机构代码、卡类别、卡种、位数）|低|
