...
|🧩 Feature|增加银行卡号校验功能|利用Luhn算法校验银行卡号有效性。点击按钮后，toast通知显示“这是有效的银行卡号，可能是Visa/万事达/JBC/Discover/美国运通/中国银联xx位（美国运通无需显示号码长度，因为运通卡号固定15位）卡片。交易前建议再次核对。”或“这是无效的银行卡号”|低|
...
|🧩 Feature|UI|恶意网页可能会通过此类手法，将恶意代码隐蔽插入复制的内容中，demo：https://thejh.net/misc/website-terminal-copy-paste 。脚本需要检测选取内的文本是否正在使用font-size: 0、使用绝对或固定定位在屏幕外、采用绝对或固定定位结合transform: scale(0)等|低|
|🧩 Feature|增加拦截网页静默向剪贴板写入内容的行为的功能|当检测到用户没有点击、复制等事件时，网页发生剪贴板写入行为。鉴于滥用剪贴板写入权限的网站非常少（因为这样容易流失大量用户，而且没什么意义），所以需要考虑是否有必要加入这个功能。如果决定实现该功能，应该将该功能设计为默认提醒+用户手动拉黑，以免误伤正常网站|待定|
...
|🧩 Feature|在输入区中增加清空按钮 | 当用户选中输入框且输入框中存在文本时，在输入框附近显示清空按钮，点击即可清空输入框中的文本。不过由于很多输入框已经自带清空按钮，需要判断是否会重复显示|待定|
...
|🧩 Feature|移除输入区的“删除按钮可见性设置项”|由于剪切=复制+删除，所以需要避免删除和剪切按钮同时出现在划词栏中造成臃肿，改为检测到input类型的输入区默认显示：复制+删除；富文本编辑器类输入区则显示：复制+剪切|待定|
