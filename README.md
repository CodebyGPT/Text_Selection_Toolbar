...
|🧩 Feature|完善“缓存已输入文本” 功能|可以进一步考虑是否可以升级为表单填充助手功能|中|
...
|🧩 Feature|优化 UI 的 Liquid Glass 效果|（同左）|低|
|🧩 Feature|超级取词功能无法读取shadow-root（open）下的文本|以msn.cn为例，用户激活超级取词功能后，鼠标无法划动选取咨询卡片中的标题文本|低|
|🧩 Feature|丰富输入区文本校对功能的规则|（同左）|低|
|🧩 Feature|在输入区中增加换行按钮 | 当用户点击输入框中任意一行的末尾时，在光标附近显示换行按钮，点击换行按钮即可在此处换行|待定|
|🧩 Feature|在输入区中增加清空按钮 | 当用户选中输入框且输入框中存在文本时，在输入框附近显示清空按钮，点击即可清空输入框中的文本|待定|
|🧩 Feature|鼠标移动至图片上方时，显示放大查看按钮，点击后支持放大查看图片|仿淘宝京东商品图片放大预览|待定|
|💡 Other|为本项目创建 GitHub Page|（同左）|待定|
|💡 Other|优化脚本代码的结构逻辑和可读性|（同左）|低|
