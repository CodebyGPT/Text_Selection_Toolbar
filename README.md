...
|🧩 Feature|增加监听并拦截非用户选词复制时网页自主向剪贴板写入内容的行为的功能|其实网页静默向剪贴板写入无用内容的情况非常少见，我个人只遇到过百度早期的移动版网页会向剪贴板写入追踪标识符这一个例子，鉴于滥用剪贴板写入权限的网站非常少（因为这样容易流失大量用户，而且没什么意义），所以需要考虑是否有必要加入这个功能。如果决定实现该功能，应该将该功能设计为默认提醒+用户手动拉黑，以免误伤正常网站|待定|
...
|🧩 Feature|鼠标移动至图片上方时，显示放大查看按钮，点击后支持放大查看图片|仿淘宝京东商品图片放大预览|待定|
